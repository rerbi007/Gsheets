...
public class InvocationActivity extends AppCompatActivity {
    ...
    EditText surname, name, patronymic, phone, email, employer;
    Button sendData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_invocation);
        ...
        sendData.setOnClickListener(v -> {
            if (isEmpty(surname.getText().toString())) {
                Toast.makeText(InvocationActivity.this, R.string.specifyLastName, Toast.LENGTH_SHORT).show();
                return;
            }
            if (isEmpty(name.getText().toString())) {
                Toast.makeText(InvocationActivity.this, R.string.specifyName, Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            if (isEmpty(email.getText().toString())) {
                Toast.makeText(InvocationActivity.this, R.string.specifyEmail, Toast.LENGTH_SHORT).show();
                return;
            }
            if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email.getText().toString()).matches()){
                Toast.makeText(InvocationActivity.this, "Невалидный email", Toast.LENGTH_SHORT).show();
                return;
            }
            if (!isCorrect.isChecked()) {
                Toast.makeText(InvocationActivity.this, R.string.confirmDataCorrectness, Toast.LENGTH_SHORT).show();
                return;
            }
            if (!iAgree.isChecked()) {
                Toast.makeText(InvocationActivity.this, R.string.acceptPersonalDataProcessing, Toast.LENGTH_SHORT).show();
                return;
            }
            if (!iAmNotRobot.isChecked()) {
                Toast.makeText(InvocationActivity.this, R.string.iAmNotRobot, Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            captcha.validate(InvocationActivity.this, request, this, sendData, progressBar);
